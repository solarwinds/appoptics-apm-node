...
function Layer (name, parent, data) {
  ...
  if (parent) {
    try {
      ...
      if (typeof parent === 'string') {
        ...
        log.layer('continuing %s layer from string %x', name, parent);
        ...
        this.parent = parent
        parent.enter()
        log.layer('continuing %s layer from event %x', name, parent.event);
      }
    } catch (e) {
      log.error(`${this.name} layer failed to set parent`, e.stack)
    }
  }
  ...
  try {
    ...
    log.error(`${this.name} layer failed to init`, e)
  }
}
...
Layer.prototype.init = function (name, parent, data) {
  // log.debug('Layer.init(%s, %x)', name, parent)
...
Object.defineProperty(Layer, 'last', {
  get () {
    ...
    try {
      ...
      log.error('Can not access cls.lastLayer. Context may be lost.')
    }
    return last
  },
  set (value) {
...
Layer.prototype.descend = function (name, data) {
  log.layer('descending layer %s from Event.last %e', name, Event.last)
...
Layer.prototype.profile = function (name, data) {
  log.layer('descending profile %s from Event.last %e', name, Event.last)
...
Object.defineProperty(Layer.prototype, 'async', {
  ...
  set (val) {
    try {
      ...
      log.layer(`${this.name} layer ${val ? 'enabled' : 'disabled'} async`)
    } catch (e) {
      log.error(`${this.name} layer failed to set async to ${val}`, e.stack)
    }
  }
})
...
Layer.prototype.runAsync = function (fn) {
  ...
  try {
    ...
    log.error(`${this.name} layer failed to enter context`, e.stack)
  }
  ...
  try {
    ...
    log.error(`${this.name} layer failed to exit context`, e.stack)
  }
...
Layer.prototype.enter = function (data) {
  let type = this instanceof Profile ? 'profile' : 'layer'
  log.force.layer('%s enter called %e', type, this.events.entry);
  ...
  try {
    ...
    log.error(`${this.name} layer failed to enter`, e.stack)
  }
}
...
Layer.prototype.exit = function (data) {
  let type = this instanceof Profile ? 'profile' : 'layer'
  log.force.layer('%s exit called %e', type, this.events.exit);
  ...
  try {
    ...
    if (last && last !== this.events.entry && !exit.ignore) {
      ...
      log.layer(`${exit} no extra edge found`)
    }

    // Send the exit event
    exit.sendReport(data)
  } catch (e) {
    log.error(`${this.name} layer failed to exit`, e.stack)
  }
}
...
Layer.prototype.setExitError = function (error) {
  try {
    ...
    log.error(`${this.name} layer failed to set exit error`, e.stack)
  }
}
...
Layer.prototype._internal = function (label, data) {
  const {last} = Event
  if (!last) {
    log.error(`${this.name} layer ${label} call could not find last event`)
    return
  }
...
Layer.prototype.info = function (data) {
  log.layer(`${this.name} layer info call`)

  try {
    // Skip sending non-objects
    if (!isRealObject(data)) {
      log.info('invalid input to layer.info(...)')
      return
    }

    this._internal('info', data)
  } catch (e) {
    log.error(`${this.name} layer failed to send info event`, e.stack)
  }
}
...
Layer.prototype.error = function (error) {
  log.layer(`${this.name} layer error call`)

  try {
    error = utility.toError(error)
    if (!error) {
      log.info('invalid input to layer.error(...)')
      return
    }

    this._internal('error', { error: error })
  } catch (e) {
    log.error(`${this.name} layer failed to send error event`, e.stack)
  }
}
