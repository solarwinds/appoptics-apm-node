...
function Layer (name, parent, data) {
  ...
  var entry = new Event(name, 'entry', valid && context)
...
Layer.prototype.run = function (fn) {
  ...
  if ( ! this.descended) {
    run = oboe.requestStore.run.bind(oboe.requestStore, run)
  }
...
Layer.prototype.enter = function (data) {
  oboe.requestStore.set('lastLayer', this)
...
Layer.prototype.exit = function (data) {
  ...
  if ( ! this.async && Event.last !== this.events.entry) {
