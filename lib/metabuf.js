...
const rb = new RandomBytes();

class Metabuf {
  constructor (metabuf) {
    this.buf = Buffer.allocUnsafe(30);
    this.buf[0] = 0x2b;
    // see if it's close enough for us.
    if (metabuf && metabuf.buf && metabuf.buf.length === 30) {
      ...
      rb.fillWithRandomBytes(this.buf, 21, 8);
...
Metabuf.makeRandom = function makeRandom (sample) {
  ...
  rb.fillWithRandomBytes(mb.buf, 1, 28);
...
Metabuf.getRandomBytesFactory = function getRandomBytesFactory () {
  return rb;
}
