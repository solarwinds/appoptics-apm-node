 ...
 * @param {object} parent Metadata to construct the event with
...
function Event (span, label, parent, edge) {
  ...
  if (!parent) {
    // leave this in while verifying full test suite.
    ao.loggers.debug('oops, no parent')
    debugger
  ...
  try {
    event = addon.Context.createEventX(parent, edge)
  } catch (e) {
    debugger
  }
  ...
  // keep KV pairs in a separate object to avoid hidden class creation.
  // define property until all KV items are stored here.
