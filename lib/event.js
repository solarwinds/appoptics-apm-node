...
const undeletedEvents = undefined;  // Map() for debugging
...
function Event (span, label, parent, edge) {
  ...
  if (undeletedEvents) {
    undeletedEvents.set(this, this.event.toString());
  }
  stats.created += 1;
  stats.active += 1;
...
Event.prototype.sendReport = function (data) {
  ...
  stats.active -= 1;
  if (undeletedEvents) {
    if (!undeletedEvents.has(this)) {
      log.debug(`expected to find ${this} in undeletedEvents`);
    } else {
      undeletedEvents.delete(this);
    }
  }
...
Event.init = function (populatedAo) {
  ...
  ao._stats.undeletedEvents = undeletedEvents;
  aob = ao.addon;
  log = ao.loggers;
