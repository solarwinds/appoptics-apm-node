...
//const {Metabuf} = require('./metabuf.js');
...
function Event (span, label, parent, edge) {
  ...
  if (edge) {
    // externally addEdge expects an Event instance but internally
    // this cheats by wrapping it so it's a skeletal Event.
    this.addEdge(parent);
...
Event.prototype.send = function (kvpairs) {
  ...
  const status = aob.Event.send(this);
