...
Event.prototype.send = function () {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (typeof val !== 'undefined') {
      logChange(this.event + ' set ' + key + ' = ' + val)
      try {
        event.addInfo(key, val)
      } catch (e) {
        logError(this.event + ' failed to set info ' + key + ' ' + value)
      }
    } else {
      logError(this.event + ' tried to set ' + key + ' to an undefined value')
    }
  }
  ...
  for (i = 0; i < len; i++) {
    var edge = edges[i]
    if ( ! edge) {
      logError(this.event + ' tried to add empty edge')
      continue
    }
    ...
    try {
      event.addEdge(edge)
      logEdge(this.event + ' added edge ' + edge)
    } catch (e) {
      logError(this.event + ' failed to add edge ' + edge)
    }
  }

  // Send the event
  if ( ! tv.reporter.sendReport(event)) {
    logError(this.event + ' failed to send to reporter')
  } else {
