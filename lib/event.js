...
function Event (layer, label, parent) {
  ...
  if (!parent) {
    ...
    // sampling decision needs to be made. this is an odd place
    // to do this but there are too many interdependencies to
    // unravel to change it all at once.
    event = addon.Context.startTrace()
    // profile sends a null layer. don't ask about sampling.
    if (layer) {
      let sample = ao.sample(layer, '')
      if (sample.sample) {
        event.setSampleFlag()
      } else {
        event.clearSampleFlag()
      }
    }
