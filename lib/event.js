...
function Event (layer, label, parent) {
  ...
  if (!parent) {
    // start a trace with new context
    ...
    // it's can be addon.Metadata, addon.Event or String.
  ...
  if (parent) {
    // TODO BAM let's figure out what types get passed in.
    log.debug('new Event() typeof parent %s', typeof parent)
