 ...
 * @param {aob.Event} parent supplies metadata to construct the event from.
...
function Event (span, label, parent, edge) {
  ...
  // aob.Event() might have added an edge.
  if (edge) {
  ...
  // there is always a "parent" event now but it might be a "foster" parent,
  // i.e., just a parent event that provides the task ID but isn't a real
  // parent. in that case "edge" is false.
  //this.parent = parent
  //if (parent) {
  //  if (parent.event) {
