...
function wrapLambdaHandler (userHandler) {
  ...
  const wrappedFunction = async function (event, context) {
    ...
    // only set span.topSpan to a specific type if an x-trace header was received.
    // if set to a specific string (not boolean true) then the span exit will attempt
    // to insert an x-trace header into the reply.
    let finalize;
    if (topSpanType) {
      finalize = (span) => {
...
function getTraceContext (headers) {
  ...
  if (ao.xtraceToEvent(headers['x-trace'])) {
...
function makeLambdaKVPairs (spanType, event, context) {
  ...
  // now it's either apig rest, v1 or v2
  ...
  // the header we are interested in should not be multivalue headers.
  const headerKeys = {
