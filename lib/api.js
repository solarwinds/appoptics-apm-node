...
function getTraceSettings (xtrace, options = {}) {
  let md;
  if (xtrace) {
    md = xtraceToBuffer(xtrace);
  }
  // if no xtrace was supplied or it was not valid make metadata and
  // clear the xtrace value. the metadata will be used to create the
  // events in the top-level span of the trace.
  if (!md) {
    md = MD.fillSkeleton();
    xtrace = '';
  }
  ...
  // if it's sampling turn on the flag.
  if (osettings.doSample) {
    MD.setFlags(md, 1);
  }
  // ignore the metadata returned by the existing code
...
function xtraceToBuffer (xtrace) {
  const b = Buffer.from(xtrace, 'hex');
