...
function definePropertiesOn (ao) {
  ...
  /**
   * Get X-Trace ID of the topSpan entry event
   *
   * @name ao.traceId
  ...
  Object.defineProperty(ao, 'traceId', {
    get () {
      ...
      if (topSpan) {
        return topSpan.events.entry.event.toString();
      }
      return undefined;
    }
  });

  Object.defineProperty(ao, 'topSpan', {
    get () {return ao.requestStore.get('topSpan');}
  })
...
function getFormattedTraceId (options = {}) {
  ...
  const topSpan = ao.requestStore.get('topSpan');
  const event = topSpan && topSpan.events.entry.event;
  return event ? event.toString(format) : '0000000000000000000000000000000000000000-0';
}
...
function insertLogObject (o = {}) {
  ...
  const topSpan = ao.requestStore.get('topSpan');
  if (ao.cfg.insertTraceIdsIntoLogs && topSpan) {
    if (ao.cfg.insertTraceIdsIntoLogs !== 'sampledOnly' || topSpan.events.entry.event.getSampleFlag()) {
