...
function wrapLambdaHandler (userHandler) {
  ...
  const wrappedFunction = async function (event, context) {
    if (ao.cfg.enabled) {
      ...
      function spanInfo () {
        return {name: 'node-lambda-span', kvpairs: mapEventInfoToKVPairs(event)};
      }
      return ao.pStartOrContinueTrace(
        inboundXTraceID,
        spanInfo,           // use function to add kvpairs
        async () => userHandler(event, context),      // promise-returning handler
        {                   // options
          forceNewTrace: true,
          lambda: {         // requestId is required for tracing lambda
...
function mapEventInfoToKVPairs (event) {
