...
function getTraceSettings (xtrace, options = {}) {
  ...
  // time not bits are defined other than the sample bit, make sure only
  // the sample bit can be turned on.
  md.assignFlags(osettings.doSample ? 0x01 : 0x00);

  // [trash-metadata] ignore the metadata returned by the existing code
...
function getFormattedTraceId (options = {}) {
  ...
  return Event.last ? Event.last.toString(format) : '0000000000000000000000000000000000000000-0';
}
...
function insertLogObject (o = {}) {
  ...
  if (ao.cfg.insertTraceIdsIntoLogs && Event.last) {
    if (ao.cfg.insertTraceIdsIntoLogs !== 'sampledOnly' || Event.last.sampling) {
