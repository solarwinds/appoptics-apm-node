...
function wrapLambdaHandler (userHandler) {
  ...
  const wrappedFunction = async function (event, context) {
    ...
    function spanInfo () {
      return {name: 'nodejs-lambda-function', kvpairs: makeLambdaKVPairs(event)};
    }
...
function promiseOnlyHandler (handler) {
  ...
  // this forces the callback to return a promise and only one of
  // the methods to complete.
  return (event, context) => {
    ...
    // if the function returns a promise then let them race so the first one
    // done is used.
    if (p && typeof p.then === 'function') {
...
function makeLambdaKVPairs (event) {
  const headers = typeof event.headers === 'object' ? event.headers : {};
  ...
  */
  if (event.httpMethod && event.headers) {
    return {
      Method: event.httpMethod ? event.httpMethod.toUpperCase() : 'GET',
      Spec: 'ws',
      URL: event.path || '/',
      Proto: headers['X-Forwarded-Proto'] || headers['CloudFront-Forwarded-Proto'] || 'https',
