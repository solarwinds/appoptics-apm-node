...
function wrapLambdaHandler (userHandler) {
  ...
  const wrappedFunction = async function (event, context) {
    ...
    if (!ao.cfg.enabled || !ao.lambda) {
      log.debug('not wrapping lambda function', ao.cfg.enabled, ao.lambda);
    ...
    let topSpanType;
    // try to find an x-trace header. if it is valid then the type
    // of request matters because we will insert an x-trace header
    // in the response if possible. if there is not a valid x-trace
    // header then topSpan is not set to a specific type and no
    // x-trace header insertion will take place at span exit.
    const inboundXTraceID = getTraceContext(event);
    if (inboundXTraceID) {
      log.debug(`lambda: found inbound x-trace ${inboundXTraceID}`);
    }
    // determine if the request comes from the api gateway.
    ...
    // only set topSpan to a specific type if an x-trace header was received.
    // if so then the span exit will attempt to insert an x-trace header on
    // the reply.
    let finalize;
    if (inboundXTraceID && topSpanType) {
      finalize = span => span.topSpan = topSpanType;
    }

    const promiseReturningHandler = promiseOnlyHandler(userHandler);
    const name = `nodejs-lambda-${userHandler.name || 'unnamed'}`;
    ...
    return ao.pStartOrContinueTrace(
      inboundXTraceID,
      ...
      userHandler.name    // no custom transaction name
    );
  }
...
function getTraceContext (event) {
  ...
  for (const key of Object.keys(rawHeaders)) {
    headers[key.toLocaleLowerCase()] = rawHeaders[key];
  }

  if (typeof headers['x-trace'] !== 'string') {
    return undefined;
  }

  const ev = ao.xtraceToEvent(headers['x-trace']);

  if (ev) {
    return headers['x-trace'];
  }

  return undefined;
}

function makeLambdaKVPairs (spanType, event, context) {
  const kvpairs = {
    Spec: typeof spanType === 'string' ? 'aws-lambda:ws' : 'aws-lambda',
  ...
  const contextProps = {
    ...
    memoryLimitInMB: 'FunctionMemorySizeMB',
  ...
  if (spanType === 'lambda-api-gateway-v1') {
    kvpairs.Method = event.httpMethod.toUpperCase();
    if (event.path) {
      kvpairs.Path = event.path;
    }
  } else if (spanType === 'lambda-api-gateway-v2') {
    kvpairs.Method = event.requestContext.http.method.toUpperCase();
    if (event.requestContext.http.path) {
      kvpairs.Path = event.requestContext.http.path;
    }
  } else {
    return kvpairs;
  }
  ...
  if (headers['X-Forwarded-Proto']) {
    kvpairs.Proto = headers['X-Forwarded-Proto'];
  } else if (headers['CloudFront-Forwarded-Proto']) {
    kvpairs.Proto = headers['CloudFront-Forwarded-Proto'];
  }
  if (headers.Host) {
    log.debug('inserting HTTP-Host header', headers.Host)
    kvpairs['HTTP-Host'] = headers.Host;
  }
  if (headers['X-Forwarded-Port']) {
    kvpairs.Port = headers['X-Forwarded-Port'];
  }
