 ...
 *
 */
function autoLambdaHandler (event, context, callback) {
  const {load} = require('/var/runtime/UserFunction');
  const taskRoot = process.env.LAMBDA_TASK_ROOT || '';
  const handler = process.env.APPOPTICS_LAMBDA_HANDLER || '';

  const userHandler = load(taskRoot, handler);

  if (!ao.cfg.enabled) {
    return userHandler;
  }

  return wrapLambdaHandler(userHandler);
}
    ...
    if (typeof event.requestContext === 'object') {
      ...
      if (event.version === '2.0' &&
        ...
        topSpanType = 'lambda-api-gateway-v2';
                ...
                typeof event.httpMethod === 'string' && typeof rc.httpMethod === 'string') {
...
function makeLambdaKVPairs (spanType, event, context) {
  ...
  if (context.invokedFunctionArn) {
    kvpairs.InvokedFunctionARN = context.invokedFunctionArn;
  }
  if (context.awsRequestId) {
    kvpairs.AWSRequestID = context.awsRequestId;
  }
