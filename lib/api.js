...
function wrapLambdaHandler (userHandler) {
  ...
  const wrappedFunction = async function (event, context) {
    ...
    const finalize = span => {
      if (typeof event.requestContext === 'object' && typeof event.requestContext.httpMethod === 'string') {
        span.topSpan = 'lambda-api-gateway';
      }
    };
    ...
    // try to find an x-trace header
    const inboundXTraceID = getTraceContext(event);
    if (inboundXTraceID) {
      log.debug(`found inbound x-trace ${inboundXTraceID}`);
    }
    function spanInfo () {
      return {name, kvpairs: makeLambdaKVPairs(event, context), finalize};
    }
...
function makeLambdaKVPairs (event, context) {
  // from go
  /*
  go api-gateway KV pairs (from collector my.appoptics.com)
  ---------------------------------------------------------
  Action:	handler
  Controller:	main
  HostHostname:	eric-go-helloworldfunction-8x1cvryr0lke
  Hostname:	eric-go-helloworldfunction-8x1cvryr0lke
  Label:	exit
  Layer:	aws_lambda
  PID:	7
  Service:	go-lambda
  TransactionName:	main.handler

  node api-gateway KV pairs (from collector my.appoptics.com)
  -----------------------------------------------------------
  HostHostname:	nodejs-apig-function-9fhbv1slutcc
  Hostname:	nodejs-apig-function-9fhbv1slutcc
  Label:	exit
  Layer:	node-lambda-span
  Service:	node-lambda-apm
  TransactionName:	custom-node-lambda-span
  */
  const kvpairs = {
    Spec: 'lambda',
  ...
  if (event.httpMethod && typeof event.headers === 'object') {
  ...
  if (context.functionName) {
    kvpairs.FunctionName = context.functionName;
  }
  if (context.functionVersion) {
    kvpairs.FunctionVersion = context.functionVersion;
  }
