...
function startOrContinueTrace (xtrace, build, run, opts, cb) {
  ...
  if (opts.lambda && opts.lambda.requestId) {
    const n = aob.Reporter.setRequestProperties({lambdaRequestId: opts.lambda.requestId});
    if (n !== 1) {
      ao.loggers.error(`failed to set lambdaRequestId (${opts.lambda.requestId})`);
    }
  }
  ...
  const wrappedFunction = async function (event, context) {
    if (ao.cfg.enabled) {
      ...
      return ao.pStartOrContinueTrace(
        inboundXTraceID,
        'node-lambda-span', // span name
        () => userHandler(event, context),      // promise-returning handler
        {                   // options
          forceNewTrace: true,
