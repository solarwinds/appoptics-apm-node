    ...
    return ret
      .then(r => {
        if (!r || typeof r !== 'object') {
          ...
          // it's v2 - supply a statusCode so lambda won't treat this object
          // as the body and add a place for the x-trace header.
          r = {statusCode: 200, body: r, headers: {}};
        } else if (span.topSpan === 'lambda-api-gateway-v2' && !('statusCode' in r)) {
          // it is an object but doesn't have a statusCode property. apig interprets this as the body and stringifies it.
        ...
        if (lastEvent) {
          debugger; // remove this and change log.debug to log.span when finished development.
...
Span.prototype.exit = function (data) {
  ...
  try {
    ...
    if (this.topSpan && ao.execEnv.id === 'lambda') {
      ...
      if (status !== 0) {
        log.warning('reporter.flush() status: %i', status);
      }
    }
  } catch (e) {
