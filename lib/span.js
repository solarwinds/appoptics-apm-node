 ...
 *                                      Events will have the same task ID and sample bit but
 *                                      unique op IDs.
 * @param {boolean} [settings.edge=true] the entry event of this span should edge back to the
 *                                       metadata. The only time this is not true is when the
 *                                       span being created is a new top level span, not being
 *                                       continued from an inbound X-Trace ID. This must be set
 *                                       to explicitly to a falsey value; it's absence is true.
 ...
 * @param {object} kvpairs key/value pairs of info to be added to the entry event
 */
