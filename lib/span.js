...
function Span (name, settings, data) {
  ...
  // is this an inbound request, e.g., http/https? if so there is no parent so just set
  // oboe's context appropriately.
  //
  // if it is not inbound then it must be descending from another span. in that case
  // metadata is the Event.last.
  try {
    let entryMetadata
    let edge = true        // if inbound and no inbound x-trace id then edge is false.
    ...
    if (settings.inbound) {
      ...
      log.span('continuing %s span from event %x', name, settings.metadata);
      this.parent = settings.metadata
      this.parent.enter()
      entryMetadata = this.parent.event
