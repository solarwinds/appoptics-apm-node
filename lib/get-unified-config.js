...
function getUnifiedConfig () {
  ...
  const settings = {
    ...
    proxy: {location: 'b', type: 's'},

    // end-user focused for lambda, both env var and config file
    serviceName: {location: 'b', type: 's', default: ''},
    // this is used only in a lambda environment. if not it's a noop.
    sampleRate: {location: 'b', type: 'i'},
    samplePercent: {location: 'b', type: 'i'},

    // end-user focused, env var only
    logLevel: {location: 'e', type: 'i', name: 'DEBUG_LEVEL', default: 2},
    logSettings: {location: 'e', type: 's', ignore: true},
    ...
    apmConfigNode: {location: 'e', type: 's', ignore: true},
  ...
  //
  const defaultConfigFile = path.join(process.cwd(), 'appoptics-apm-config')
  ...
  //
  // now validate the serviceKey or serviceName.
  //
  if (execEnv.type === 'serverless' && execEnv.id === 'lambda') {
    // there must be a valid service name
    const serviceName = results.global.serviceName;
    const cleansedName = cleanseServiceName(serviceName);
    if (!validServiceName(cleansedName)) {
      fatals.push(`not a valid serviceName: ${serviceName}`);
      results.global.serviceName = '';
    //} else if (!validServiceName(serviceName)) {
    } else if (cleansedName !== serviceName) {
      warnings.push(`serviceName specified: ${serviceName} converted to: ${cleansedName}`);
      results.global.serviceName = cleansedName;
    } else {
      debuggings.push(`using serviceName: ${serviceName}`);
    }
    ...
    //} else if (!validKey(serviceKey)) {
      ...
      debuggings.push(`using serviceKey ${mask(serviceKey)}`);
    }
    // there may be only one
    delete results.global.serviceName;
  }
...
function getExecutionEnvironment () {
  ...
  return {type: 'linux', id: undefined, nodeEnv: ne};
}
...
function validServiceName (name) {
  return !!name.match(/^[A-Za-z0-9.:_-]{1,255}$/);
}
