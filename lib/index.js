...
// specified then let that override the default.
...
exports.specialUrls = config.transactionSettings.filter(s => s.type === 'url')
...
if (!enabled) {
  ...
  exports.getTraceSettings = function (xtrace, localMode) {
    ...
    if (!bindings.Context.getTraceSettings) {
      ...
      // if there is no localMode return settings as they are.
    ...
    return bindings.Context.getTraceSettings({
      xtrace,
      mode: localMode,
      rate: sampleRate,
    })
  ...
  exports.stringToMetadata = function (xtrace) {
    ...
    if (!md) {
      log.warn(`invalid X-Trace string "${xtrace}"`)
    }
    return md
  }
  ...
  exports.startOrContinueTrace = function (xtrace, build, run, opts, cb) {
    ...
    try {
      ...
      if (xtrace) {
        // stringToMetadata returns undefined if it fails so it will start
        // a trace as opposed to continuing one.
        settings.metadata = ao.stringToMetadata(xtrace)
        settings.metadata.setSampleFlagTo(settings.doSample)
      }
    } catch (e) {
      log.error('ao.startOrContinueTrace can\'t get a sample decision', e.stack)
      settings = {inbound: true, doSample: false}
    }

    // Now actually make the span
    let span
    try {
      span = new maker.cons(maker.name, settings, maker.data)

      // Add sampling data to entry if there was not already an xtrace ID
      if (settings.sample && !xtrace) {
  ...
  // This is a helper to map span.descend(...) to the data provided to them,
  // rather than producing spans directly. This allows acquiring the span
  // name before sampling but only creating the span after sampling.
