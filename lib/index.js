 ...
 * @return {string|undefined} - the log levels in effect or undefined if an error
 *
 * @example
 ...
 * @return {string|undefined} - log levels active after removals or undefined if an error.
 * @example
...
exports.readyToSample = function (ms, obj) {
  let status = exports.reporter.isReadyToSample(ms)
 ...
 * @return {function} The bound function or the original function if it can't be bound.
 */
exports.bind = function (fn) {
  try {
    ...
    if (!clsCheck()) {
      ...
      log.warn('ao.bind %s - not a function', name)
    }
    return fn
  } catch (e) {
 ...
 * @return {EventEmitter} The bound emitter or the original emitter if it can't be bound.
 */
exports.bindEmitter = function (em) {
  try {
    if (em && typeof em.on === 'function') {
      ...
      // has been setup when the http instrumentation binds the events but there must be
      // CLS context.
 ...
 * @param {function} fn - A function that returns a string name to use or a falsey
 *                        value to use the default. The calling signature of the
 *                        function varies by the probe.
 *                        Pass a falsey value instead of a function to clear.
 */
exports.setCustomTxNameFunction = function (probe, fn) {
  // if the probe exists set the function and return success
  if (probe in exports.probes) {
...
/**
 * Set the custom transaction name for a context associated with an http response.
 *
 * This is commonly used when custom names are desired for a small subset of the routes.
 * TODO - needs reworked to handle non-HTTP traces.
 *
 * @ignore
...
if (!enabled) {
   ...
   *
   * @method ao.sampling
   * @param {string|Event|Metadata} item - the item to check the sampling flag of
   ...
   * @return {bindings.Metadata|undefined} - bindings.Metadata object if successful.
   */
