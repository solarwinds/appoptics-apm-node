...
// read the config file first so that if it disables the appoptics
// bindings are not loaded.
const path = require('path')
const log = require('./loggers')

let configFile = process.env.APPOPTICS_CONFIG_FILE || path.join(process.cwd(), 'appoptics-apm')
...
try {
  ...
  if (e.code !== 'MODULE_NOT_FOUND') {
    console.warn(`error reading config file ${configFile}`)
  }
}
exports.cfg = {}
...
Object.defineProperty(exports, 'sampleMode', {
  ...
  set (value) {
    ...
    log.settings('set sample mode to ' + value)
    if (enabled) {
...
if (!enabled) {
  ...
  function runInstrument (last, make, run, options, callback) {
    ...
    try {
      ...
      error('ao.runInstrument failed to build layer', e.stack)
    }
