...
const log = debug('appoptics:settings')
...
if (!enabled) {
  ...
  process.nextTick(function () {
    exports.requestStore.run(function () {
      ...
      /*
      init('making event')
      const e = new Event('nodejs', 'single', null)
      init('sending status')
      e.sendStatus(data)
      init('init complete')
      bindings.Context.set(e.event)
      // */
      //*
      let originalReporter = Event.prototype.sendReport
      init('creating new layer for init')
      const layer = new Layer('nodejs', null, data)

      init('init layer enter()')
      Event.prototype.sendReport = Event.prototype.sendStatus
      layer.enter()

      init('init layer exit()')
      Event.prototype.sendReport = function () {return 0}
      layer.exit()

      Event.prototype.sendReport = originalReporter
      init('init complete')
      // */
      /*
