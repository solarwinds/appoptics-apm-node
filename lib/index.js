...
if (!enabled) {
  ...
  function runInstrument (last, make, run, options, callback) {
    // Build layer
    const layer = typeof make === 'function' ? make(last) : last.descend(make)
  ...
  exports.startOrContinueTrace = function (xtrace, build, run, opts, cb) {
    ...
    // Build data
    const data = typeof build === 'function' ? build({
    ...
    const layer = new data.cons(data.name, xtrace, data.data)

    // Add sampling data to entry
    if (!xtrace) {
      const entry = layer.events.entry
