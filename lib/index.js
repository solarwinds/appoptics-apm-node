...
let config = require('./parse-config-file')(configFile, './probe-defaults')


if (config.configError) {
  if (config.configError !== 'MODULE_NOT_FOUND' || configFile !== defaultConfigFile) {
    log.error('Cannot read config file %s', configFile, config.configError)
  }
}

if (config.defaultsError) {
  log.error('Cannot read probe defaults ./lib/probe-defaults', config.defaultsError)
}

exports.probes = config.probes
exports.cfg = config = config.config
