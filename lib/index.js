...
// check APPOPTICS_LOG_SETTINGS (not oboe settings).  APPOPTICS_LOG_SETTINGS
// take precedence over DEBUG settings. But if APPOPTICS_LOG_SETTINGS is not present
// then pay attention to DEBUG.
const nonAoSettings = []
let aoSettings = []
if (env.DEBUG) {
  const debugSettings = env.DEBUG.split(',').map(s => s.trim())
  // sort them into those intended for appoptics and those not.
  debugSettings.forEach(s => {
    (s.indexOf('appoptics:') === 0 ? aoSettings : nonAoSettings).push(s)
  })
  // remove appoptics settings but put non-appoptics settings back; appoptics
  // settings will be added back if APPOPTICS_LOG_SETTINGS is not defined.
  env.DEBUG = nonAoSettings.join(',')
}

// if set this take precedence over DEBUG so replace any aoSettings already found.
if ('APPOPTICS_LOG_SETTINGS' in env) {
  aoSettings = env.APPOPTICS_LOG_SETTINGS.split(',').map(s => s.trim())
}
...
if (aoSettings.length) {
  logLevel = aoSettings.map(s => s.slice('appoptics:'.length))
} else {
  logLevel = ['error', 'warn']
}
...
if (alreadyLoaded.length && env.NODE_ENV === 'production') {
