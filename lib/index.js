...
//
exports.instrument = function (run, callback, options, descender) {
  var last = Layer.last
  if ( ! last) {
    return run(callback)
  }
  ...
  if ( ! options.enabled) {
    return run(exports.requestStore.bind(callback))
  }

  var layer = descender(last)
...
exports.instrumentSync = function (run, options, descender) {
  var last = Layer.last
  if ( ! last) {
    return run()
  }
