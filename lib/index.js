...
// now that the config is known warn about files already being required if this is production.
if (alreadyLoaded.length && ao.execEnv.nodeEnv === 'production') {
...
// if the key/name is not valid then the agent cannot be enabled.
if (ao.execEnv.type === 'serverless' && ao.execEnv.id === 'lambda') {
  enabled = enabled && config.serviceName;
} else {
...
if (enabled) {
  ...
  if (ao.execEnv.type !== 'serverless' || ao.execEnv.id !== 'lambda') {
    ...
    delete options.samplePercent;
  }

  /* disable until requiring 'notifications' spikes memory is resolved.
  const semverSatisfies = require('semver/functions/satisfies');

  const badVersion = semverSatisfies(process.version, '>=12.0.0 <12.17.0');

  //
  // now start the notification service before initializing
  // oboe (which might generate log messages).
  //
  if (options.unifiedLogging === 'preferred' && !badVersion || options.unifiedLogging === 'always') {
    const Notifications = require('./notifications');
    const notificationListener = require('./notifications-listener');
    ao.notifications = new Notifications(notificationListener, bindings);
    ao.notifications.startNotifier();
  } else {
    let msg = 'unified logging disabled by configuration';
    if (options.unifiedLogging === 'preferred') {
      msg += ` for node ${process.version}`;
    }
    log.debug(msg);
  }
  // */
  ...
  if (ao.execEnv.id === 'lambda' && bindings.Reporter.getType() !== 'lambda') {
    log.error(`execution environment mismatch ${ao.execEnv.id} vs. ${bindings.Reporter.getType()}`);
  }
  // samplePercent takes priority because it was added after sampleRate, so if it's being used
  // a conscious decision was made to use it.
  if (ao.cfg.samplePercent !== undefined) {
    const rateUsed = bindings.Settings.setDefaultSampleRate(ao.cfg.samplePercent * 10000) / 10000;
    if (rateUsed !== ao.cfg.samplePercent) {
      log.warn(`sample percent request ${ao.cfg.samplePercent}, used: ${rateUsed}`);
    }
  } else if (ao.cfg.sampleRate !== undefined) {
    const rateUsed = bindings.Settings.setDefaultSampleRate(ao.cfg.sampleRate);
    if (rateUsed !== ao.cfg.sampleRate) {
      log.warn(`sample rate requested ${ao.cfg.sampleRate}, used: ${rateUsed}`);
    }
  }
  ...
  ao.fs = require('fs');
  // clean up any leftover socket paths
  if (ao.notifications) {
    ao.fs.readdir(ao.notifications.socketDir, function (e, files) {
      if (e) {
        return;
      }
      const dir = ao.notifications.socketDir;
      const fileOnlyPrefix = ao.notifications.socketPrefix;
      files.filter(f => f.startsWith(fileOnlyPrefix))
        .forEach(f => {
          // if it's the currently active socket ignore it
          if (ao.notifications.socket.endsWith(f)) {
            return;
          }
          // delete the file ignoring any errors.
          ao.fs.unlink(`${dir}${f}`, () => undefined);
        });
    });
  }
  ...
  nextTick(function () {
    ...
    ao.requestStore.run(function () {
      // save the fs probe and disable it for startup.
      const originalFsProbeConfig = ao.probes.fs;
      ...
      ao.probes.fs = originalFsProbeConfig;
      startup = false
