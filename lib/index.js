...
if (!bindings) {
  enabled = false
  bindings = require('./addon-essentials')
...
if (!enabled) {
  ...
  exports.getTraceSettings = function (xtrace, localMode) {
    ...
    const log = !!exports.control.logging.getTraceSettings
    if (log) {
      settings.log = true
    }
    ...
    if (log) {
      const mode = osettings.mode || 'unset'
      const s = osettings.doSample
      const m = osettings.doMetrics
      const r = osettings.rate
      const source = osettings.source
      console.log(`x: ${xtrace || 'none'} lm ${mode}; s ${s} m ${m} r ${r} source: ${source}`)
    }
