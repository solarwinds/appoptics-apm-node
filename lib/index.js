...
if (enabled) {
  ...
  const goodVersion = semverSatisfies(process.version, '>=12.0.0 <12.17.0');
  //
  // now start the notification service before initializing
  // oboe (which might generate log messages).
  //
  if (options.unifiedLogging === 'preferred' && goodVersion || options.unifiedLogging === 'always') {
    const Notifications = require('./notifications');
    const notificationListener = require('./notifications-listener');
    ao.notifications = new Notifications(notificationListener);
    ao.notifications.startNotifier();
  } else {
    log.debug('notification provider disabled by AO_TEST_NO_NOTIFICATIONS');
  }
