...
if (!enabled) {
  ...
  exports.getTraceSettings = function (xtrace, localMode) {
    ...
    let osettings
    if (Date.now() - lastError5 > 60000) {
      log.debug('[asking oboe for settings]')
      osettings = bindings.Context.getTraceSettings(settings)
    } else {
      osettings = {error: 99, message: 'waiting for Godot'}
    }
    ...
    if (osettings.error) {
      ...
      log.debug('[got non-error return]')
    }
