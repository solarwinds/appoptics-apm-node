...
if (process.env.DEBUG) {
  logLevel = process.env.DEBUG.split(',').filter(p => p.startsWith('appoptics:'))
...
Object.defineProperty(exports, 'logLevel', {
  ...
  set(value) {
    ...
    if (Array.isArray(value)) {
      let notao = (process.env.DEBUG || '').split(',').filter(p => !p.startsWith('appoptics:'))
      logLevel = value
...
// if appoptics is not specified in DEBUG then default
if (!('DEBUG' in process.env) || !~process.env.DEBUG.indexOf('appoptics:')) {
...
try {
  ...
  config = {}
  // if not found only log an error if it is a user specified file, not the default
...
if (enabled) {
  try {
    ...
    log.error('Could not load liboboe native bindings, disabling AppOptics\n\n', e.stack)
    enabled = false
  }
}
...
if (!enabled) {
  ...
  if (exports.cfg.hostnameAlias) {
    options.hostnameAlias = exports.config.hostnameAlias
  }
