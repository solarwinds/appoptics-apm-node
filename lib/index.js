...
try {
  defaults.probes = require('./probe-defaults').probes
} catch (e) {
...
if (!serviceKey) {
  ...
  if (config.serviceKey) {
    log.debug(`using serviceKey from ${configFile}`)
...
// undefined is only valid in the config file.
...
//
if (enabled) {
  // get the environment variables for initializing c-lib.
  const {valid, invalid} = require('./c-lib-env-vars').fetch();
  const options = valid;

  if (invalid.length) {
    log.warn('unknown APPOPTICS_ env variables:', invalid.join(','));
  }
  ...
  // add a config file hostnameAlias if not specified in the environment
  if (ao.cfg.hostnameAlias && !options.hostnameAlias) {
    options.hostnameAlias = ao.cfg.hostnameAlias
  }

  // perform a little sleight of hand for testing.
  if (options.reporter === 'udp' && env.APPOPTICS_REPORTER_UDP) {
    options.endpoint = env.APPOPTICS_REPORTER_UDP;
  }

  // override config file if present in options returned.
  if ('triggerTrace' in options) {
    ao.cfg.triggerTraceEnabled = options.triggerTrace;
  }
