...
//
let sampleMode, sampleRate, sampleSource, host, port, accessKey
...
Object.defineProperty(exports, 'sampleMode', {
  ...
  set (value) {
    ...
    log.settings('set tracing mode to ' + value)
    if (enabled) {
...
let config
try {
  config = require(path.join(process.cwd(), 'appoptics'))
...
//
if (!accessKey) {
  const aoServiceKey = process.env.APPOPTICS_SERVICE_KEY
  if (aoServiceKey) {
    exports.accessKey = aoServiceKey
  } else {
    ...
    if (fs.existsSync(configFile)) {
      ...
      while ((line = lines.shift())) {
        if (/^appoptics.service_key=/.test(line) || /^service_key/.test(line)) {
...
if (!enabled) {
  ...
  exports.sampling = function (item) {
    ...
    if (item instanceof addon.Metadata) {
      return item.getSampleFlag()
    }

    throw new Error("Sampling called with " + typeof item)
  }
  ...
  exports.instrument = function (build, run, options, callback) {
    ...
    try {
      ...
      error('ao.instrument failed to normalize arguments', e.stack)
    }
  ...
  function runInstrument (last, make, run, options, callback) {
    ...
    try {
      ...
      error('ao.instrument failed to build layer', e.stack)
    }
  ...
  exports.startOrContinueTrace = function (xtrace, build, run, opts, cb) {
    ...
    try {
      ...
      error('ao.startOrContinueTrace failed to normalize arguments', e.stack)
    }
    ...
    try {
      ...
      error('ao.startOrContinueTrace failed to build layer', e.stack)
    }
    ...
    try {
      ...
      error('ao.startOrContinueTrace failed to sample', e.stack)
    }
  ...
  process.nextTick(function () {
    exports.requestStore.run(function () {
      ...
      // */
      /*
      let originalReporter = reporter.sendReport
      init('creating new layer for init')
      const layer = new Layer('nodejs', null, data)

      init('init layer enter()')

      reporter.sendReport = reporter.sendStatus
      layer.enter()

      init('init layer exit()')
      reporter.sendReport = function () {return 0}
      layer.exit()

      reporter.sendReport = originalReporter
      init('init complete')
      // */
      /*
      reporter.initDone = false
      init('creating new Layer for init')
      const layer = new Layer('nodejs', null, data)
      init('init layer enter()')
      layer.enter()
