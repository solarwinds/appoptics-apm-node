...
if (!enabled) {
  ...
  exports.getTraceSettings = function (xtrace, localMode) {
    ...
    if (!bindings.Context.getTraceSettings) {
      ...
      // oboe really needs to be involved.)
    ...
    const settings = bindings.Context.getTraceSettings({
      xtrace,
      mode: localMode,
      rate: sampleRate,
    })
   ...
   * @param {string|function} build - span name or span-info function
   ...
   *     properties may exist - see instrumenting-a-module.md in guides/.
  ...
  function runInstrument (last, make, run, options, callback) {
    ...
    if (!~['function', 'string'].indexOf(typeof make)) {
      log.warn('ao.runInstrument found no span name or builder function')
      return run(callback)
    }
