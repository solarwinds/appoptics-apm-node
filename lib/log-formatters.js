...
d.formatters.x = xid => {
  ...
  if (typeof xid === 'object') {
    if (xid.toString() === '[object Object]') {
      // is this a layer? if so return name
      if ('descended' in xid) {
        return `<layer: ${xid.name}>`
      }
      // show the keys as a clue
      return `<unknown: ${Object.keys(xid)}>`
    }
    xid = xid.toString()
    prefix = 'ot '
  }
  return prefix + xid.slice(2, 42) + ', o ' + xid.slice(42, 58) + ', f ' + xid.slice(-2)
}
...
function humanID (x) {
  ...
  if (!(x instanceof aob.Event) && !(x instanceof aob.Metadata)) {
    if (typeof x === 'string' && x.length === 60) {
      return x.slice(0, 2) + ':' + x.slice(2, 42) + ':' + x.slice(42, 58) + ':' + xid.slice(-2)
    }
    // it's not a well formatted string, so return some indication of that.
    return '-:-:-:-'
  }
  return x.toString(1)
}
