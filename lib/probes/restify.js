...
function patchServerHandle (server) {
  if (typeof server._handle !== 'function') {
    ao.loggers.debug('server._handle not a function')
    ao.loggers.debug('proto:', Object.getPrototypeOf(server))
    ao.loggers.debug('server constructor: ', server.constructor)
    return
  }
  ...
  return function (...args) {
    ...
    return ao.instrumentHttp(
      last => {
        ...
        // NOTE: restify has no "controller" concept, url pattern will suffice
...
function patchServerMethod (server, method) {
  if (typeof server[method] !== 'function') {
    ao.loggers.debug('server method ' + method + ' not a function')
    return
  }
...
function patchCreateServer (restify) {
  if (typeof restify.createServer !== 'function') {
    ao.loggers.debug('createServer not a function')
    return
  }
  shimmer.wrap(restify, 'createServer', fn => function () {
    const server = fn.apply(this, arguments)
    ao.loggers.debug('server keys: ' + Object.keys(server))
    patchServer(server)
    return server
  })
}
