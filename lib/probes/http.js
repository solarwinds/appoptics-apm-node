    ...
    ao.requestStore.run(() => {
      ...
      try {
        ...
        ret.prependListener('upgrade', (res, socket, head) => {
          ...
          //
          // the upgrade event is only emitted if code is 101 so that probably
          // could be hardcoded. but the message could vary so best to
          span.exit({
...
function patchServer (module, options, protocol) {
  ...
  shimmer.wrap(proto, 'emit', realEmit => function (type, req, res) {
    // commented out pending decision on instrumenting the server side of
    // upgrade requests.
    //if (!conf.enabled) {
