...
function patchServer (module, protocol) {
  ...
  function getRequestHeaders (layer, req) {
    ...
    // Add some extra KV pairs to the entry event of the layer
  ...
  function wrapRequestResponse (layer, req, res) {
    ...
    ao.addResponseFinalizer(res, () => {
      ...
      // BAM here to calculate and send inbound metrics values
      let ok = ao.reporter.sendHttpSpanUrl(
        req.url,
        (new Date().getTime() - res._ao_metrics_start) * 1000,
        res.httpStatus,
        req.method,
        res.httpStatus >= 500 && res.httpStatus <= 599
      )
      if (!ok) {
        log.error('sentHttpSpanUrl() failed')
      }
    })
