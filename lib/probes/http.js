  ...
  const wrapper = fn => function (...args) {
    ...
    ao.requestStore.run(() => {
      ...
      try {
        ...
        // make sure the span is exited when aborted or a socket error. could
        // patch ret.emit but prepending listeners uses a documented mechanism
        // so is likely to be more reliable in the long run.
        ret.prependListener('error', error => {
          span.error(error);
          span.exit();
        });
        ret.prependListener('abort', () => {
          span.exit();
        });
