...
function patchServer (module, protocol) {
  ...
  shimmer.wrap(proto, 'emit', realEmit => function (type, req, res) {
    ...
    // i.e., forcing a trace and/or supplying metadata to be associated
    // with a trace.
    const xtraceOpts = req.headers['x-trace-options'];
    const xtraceOptsHash = {};
    const xtraceOptsSig = req.headers['x-trace-options-mac'];
    ...
    if (xtraceOpts) {
      ...
      if (xtraceOptsHash.ts) {
        xtraceOptsHash.ts = Number(xtraceOptsHash.ts);
        if (Number.isNaN(xtraceOptsHash.ts)) {
          xtraceOptsHash.ts = 0;
        }
    ...
    // these settings are dual-purpose. they are an argument to getTraceSettings()
    // and they contain information that is required by wrapRequestResponse() for
    // returning headers to the client and setting KV pairs in the span.
    const settingsOptions = {
      ttRequested,
    };

    // if trigger-trace set tt options and no need to check whether there
    // is any URL-specific filtering going on.
    if (ttRequested) {
      settingsOptions.ttOptions = xtraceOpts || '';
      settingsOptions.ttSignature = xtraceOptsSig || '';
      settingsOptions.ttTimestamp = xtraceOptsHash.ts || 0;
    } else {
      settingsOptions.mode = getUrlFilterMode(req);
    }

    // get sample & metrics decision.
    const settings = ao.getTraceSettings(xtrace, settingsOptions);

    const ttApproved = ttRequested && settings.status <= 0;
    const authApproved = settings.authStatus <= 0;
    ...
    if (ao.Event.last) {
      log.debug('unexpected Event.last %e', ao.Event.last);
    ...
    ao.requestStore.run(() => {
      try {
        ...
        // if an x-trace-options header set x-trace-options-response header
        if (xtraceOpts) {
          // set the options-response header to either trigger-trace status code or not requested
          xtraceOptsResponse = `trigger-trace=${ttRequested ? settings.message : 'not-requested'}`;

          // now add known keys to the span's kvpairs
          if (xtraceOptsHash['pd-keys']) {
            kvpairs['pd-keys'] = xtraceOptsHash['pd-keys'];
            delete xtraceOptsHash['pd-keys'];
          }
          for (let k in xtraceOptsHash) {
          ...
          // only add auth key to the response header if an x-trace-options-signature header
          if (xtraceOptsSig) {
            xtraceOptsResponse += `;auth=${settings.authMessage}`;
          }

          // finally add any ignored keys to the response header
          const ignored = Object.keys(xtraceOptsHash).join(',');
          if (ignored) {
            xtraceOptsResponse += `;ignored=${ignored}`;
          }
        }
  ...
  function wrapRequestResponse (span, req, res) {
    ...
    let active = ao.requestStore.active;

    if (ao.Event.last) {
      log.debug('wrapRequestResponse: unexpected Event.last %e', ao.Event.last);
    }
    ...
    ao.addResponseFinalizer(res, ao.requestStore.bind(() => {
      const {last} = Event
      log.debug('responseFinalizer: Event.last %e', last);
