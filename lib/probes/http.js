...
function patchServer (module, protocol) {
  ...
  function wrapRequestResponse (span, req, res) {
    ...
    ao.addResponseFinalizer(res, () => {
      ...
      if (res._ao_metrics.txname) {
        f = 'sendHttpSpanName'
        // attach the transaction name to the exit event and replace the url
        // in urlOrName.
        exitKeyValuePairs.TransactionName = urlOrName = res._ao_metrics.txname
      }

      if (res._ao_metrics.customNameFuncCalls > 1) {
        log.debug('customNameFuncCalls %d', res._ao_metrics.customNameFuncCalls)
      }
