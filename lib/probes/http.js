...
function patchClient (module, protocol) {
  ...
  let wrapper = fn => function (options, callback) {
...
function patchServer (module, protocol) {
  ...
  function wrapRequestResponse (span, req, res) {
    ...
    ao.addResponseFinalizer(res, () => {
      ...
      let args = {
        transaction: res._ao_metrics.txname,
        url: req.url,
        domain: ao.cfg.domainPrefix ? req.headers.hostname : undefined,
      ...
      // falsey on an error.
      let txname = ao.reporter.sendHttpSpan(args);
      ...
      if (txname) {
        ...
        log.debug('txname default %s, requested %s, final %s, calls %s',
          exitKeyValuePairs.TransactionName,
          args.transaction,
          txname,
          res._ao_metrics.customNameFuncCalls
        )
      } else {
        log.error('sendHttpSpan() failed')
      }
