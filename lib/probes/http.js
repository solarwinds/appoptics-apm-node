...
function patchServer (module, protocol) {
  ...
  shimmer.wrap(proto, 'emit', realEmit => function (type, req, res) {
    ...
    if (xtraceOpts && ao.cfg.triggerTraceEnabled) {
    ...
    ao.requestStore.run(() => {
      try {
        ...
        if (xtraceOpts) {
          ...
          if (!ao.cfg.triggerTraceEnabled) {
            responseParts.push('trigger-trace=trigger-trace-disabled');
          } else {
            ...
            // if the auth status is "good" add other trigger trace response items.
