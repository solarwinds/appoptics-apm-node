...
function patchServer (module, protocol) {
  ...
  shimmer.wrap(proto, 'emit', realEmit => function (type, req, res) {
    ...
    // URL filter mode doesn't apply if a trigger-trace is requested.
    if (!ttRequested) {
      settingsOptions.mode = getUrlFilterMode(req);
    }

    settingsOptions.showIn = false && xtraceOptsSig;

    // get sample & metrics decision.
    const settings = ao.getTraceSettings(xtrace, settingsOptions);

    //xtraceOptsSig && console.log(util.inspect(settings));
