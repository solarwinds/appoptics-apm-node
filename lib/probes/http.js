    ...
    ao.requestStore.run(() => {
      ...
      try {
        ...
        ret.prependListener('upgrade', (res, socket, head) => {
          // report info on upgrade
          span.info({
            Message: 'Upgrading HTTP connection to Websockets.',
            HTTPStatus: 101 // switching protocols
          });
          // and exit the span immediately. it's not possible
          // to know how long a websocket session will exist.
          // documentation-wise we'll need to note that we only
          // instrument the upgrade and that any instrumentation of
          // transactions over an open web socket must use the custom
          // instrumentation API.
          span.exit();
        });
...
function patchServer (module, options, protocol) {
  ...
  shimmer.wrap(proto, 'emit', realEmit => function (type, req, res) {
    ...
    if (type === 'upgrade') {
      // process
      // args are: req, socket
      // capture req information including websocket-specific headers
      // create span with additional kv pairs
      // enter followed by immediate exit
      // i.e., don't try to capture streamed response written to socket
      //host: 'localhost:3210',
      //connection: 'Upgrade',
      //pragma: 'no-cache',
      ...
      //cookie: '_ga=GA1.1.1264382260.1545229952',
      //'sec-websocket-key': 'QxPyUJrIt/qH+8gcO7vMIQ==',
