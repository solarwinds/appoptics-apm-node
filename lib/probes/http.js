...
function patchServer (module, protocol) {
  ...
  shimmer.wrap(proto, 'emit', realEmit => function (type, req, res) {
    ...
    //const settings = ao.getTraceSettings('nodejs', xtrace)
    const settings = ao.getTraceSettings(xtrace, localMode)
    // set inbound true so that this will start a trace with, or without, an
    // incoming x-trace ID.
    settings.inbound = true

    // if there is an xtrace then we have valid metadata in md. make sure md's
    // sample bit is set correctly, and add the metadata to the settings.
    if (xtrace) {
  ...
  function makeSpan (req, settings) {
    return new Span('nodejs', settings, {
      'Spec': 'ws',
  ...
  function getRequestHeaders (span, req) {
    ...
    if (ao.sampling(entry)) {
      entry.kv.SampleRate = ao.sampleRate
      entry.kv.SampleSource = ao.sampleSource
    }
  }
...
function getUrlFilterMode (req) {
  ...
  for (const s of ao.specialUrls) {
    if (s.url) {
      ...
      ao.loggers.warn('url-specific filter is missing url and regex properties')
    }
  }
