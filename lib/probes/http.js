...
function patchClient (module, protocol) {
  ...
  const wrapper = fn => function (...args) {
    ...
    // the first argument might be a string or an urlURL
    ...
    ao.requestStore.run(() => {
      ...
      // Do request. args may be different than the caller's original args
      // if not options argument was supplied because we need to add an
      // x-trace header.
      ret = fn(...args)
