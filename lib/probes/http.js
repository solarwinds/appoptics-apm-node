...
function patchServer (module, protocol) {
  ...
  shimmer.wrap(proto, 'emit', realEmit => function (type, req, res) {
    ...
    // get a sampling decision but we're going to trace
    // no matter what.
    let sample = ao.sample('nodejs', xtrace)
    let md

    // if there is an xtrace header try to convert it to
    // metadata.
    if (xtrace) {
      md = ao.stringToMetadata(xtrace)
      // if it isn't valid metadata set metadata to empty
      // string. it's a kludge but the .toString() later is
      // a noop and it will pass and empty string.
      if (!md) {
        log.info('invalid X-Trace header received: %s', xtrace)
        md = ''
      }
    } else {
      md = ao.addon.Metadata.makeRandom(sample.sample)
    }

    // if there wasn't an xtrace header OR the xtrace header
    // was invalid then create metadata with the sample flag
    // set appopriately.
    /*
    if (!md && !xtrace) {
      md = ao.addon.Metadata.makeRandom(sample.sample)
    ...
    ao.requestStore.run(() => {
      try {
        // TODO BAM make this use md, not md.toString()
        const layer = res._http_layer = makeLayer(req, md.toString())
