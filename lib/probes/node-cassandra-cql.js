...
const conf = ao['node-cassandra-cql']
    ...
    shimmer.wrap(proto, method, fn => function (...args) {
      ...
      return ao.instrument(
        last => {
          ...
          return (layer = last.descend('cassandra', data))
        },
        done => fn.apply(this, args.concat(function (err) {
          reportErrors(layer, err)
...
function reportErrors (layer, err) {
  if (layer && err instanceof Error && Array.isArray(err.individualErrors)) {
    err.individualErrors.forEach(err => layer.error(err))
  }
}
