...
function routeProfiling (express, version) {
  ...
  function buildSpan (last, req, res, func) {
    const httpSpan = res._http_span

    // Determine controller and action values
    var Controller = 'express'
    var Action = req.method + req.route.path

    // if the end user is potentially overriding the metrics name
    if (conf.makeMetricsName) {
      let name
      try {
        name = conf.makeMetricsName(req, res)
      } catch (e) {
        log.error('express makeMetricsName:', e)
      }
      if (name && name.Controller && name.Action) {
        Controller = name.Controller
        Action = name.Action
      }
    }
    // rewrite the name used for the metrics
    let name = `${Controller}.${Action}`
    res._ao_metrics.name = name

    // now set them back to what the profile used.
    Controller = req.route.path
    Action = utility.fnName(func)
      ...
      return ao.instrumentHttp(
        ...
        () => {
          ...
          try {
            ...
            log.error('express - failed to patch span exit')
          }
        ...
        return ao.instrument(
          last => {
            ...
            if (ao.rumId) {
              ...
              rum.inject(args[0], ao.rumId, topSpan.events.exit)
            }
            // */
