...
function patchClient (proto, Query) {
  ...
  shimmer.wrap(proto, 'query', fn => function (...args) {
    ...
    // QQuery so it doesn't mask the argument Query.
    const QQuery = typeof sql === 'object' ? sql.sql : sql
    const QueryArgs = typeof values !== 'function' ? values : sql.values

    // Set basic k/v pairs. note that database is defaulted to ''
    // in kvpairs because not all queries' config contains a
    ...
    if (conf.sanitizeSql) {
      ...
      kvpairs.QueryArgs = JSON.stringify(
