 ...
 * o drop_collection
 * o drop_index
 * - drop_indexes (No such query, drop_index is used with patterns)
 * - ensure_index (No such query, compound of index_inforimation and create_index)
 * o find
 * o find_and_modify
 * - group
 * o index_information
 * - inline_map_reduce (No such query, does not appear to exist...)
 * o insert
 * o map_reduce
 * - options (No such query, must patch abstract interface)
 * o reindex
 * o remove
 * o rename
...
function identify (db_command) {
  ...
  // If it has a renameCollection key, it is a rename command
    ...
    data.QueryOp = 'rename'
    data.Query = {}
    data.New_Collection_Name = query.to
    data.Collection = query.create.replace(reg, '')
  ...
  // If it has an mapreduce key, it is an map_reduce command
    ...
    data.Query = {}
    data.Map_Function = query.map.toString()
    data.Reduce_function = query.reduce.toString()
  ...
  // If it has an update key, it is an update command
    ...
    data.Update_Document = query.document
